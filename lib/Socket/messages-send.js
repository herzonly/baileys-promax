...
const { isJidGroup } = require('../WABinary');
...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, statusJidList, AI = false }) => {
    ...
    if (isJidGroup(jid)) {
        try {
            ...
            const expiration = metadata?.ephemeralDuration || Date.now()
            options.ephemeralExpiration = expiration
        } catch (e) {
            options.ephemeralExpiration = Date.now()
        }
    }
}
