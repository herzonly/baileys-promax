...
const { resolveJidFromLid, createLidToJidMap } = require('../Utils/generics');
...
const makeMessagesSocket = (config) => {
    ...
    const convertLidToJid = (id) => {
    if (id.includes('@lid')) {
        if (id.includes('-') && id.includes('@lid')) {
            return id.replace('@lid', '@g.us');
        }
    ...
    let lidToJidCache = new Map();

const getCachedLidMapping = (groupJid) => {
    return lidToJidCache.get(groupJid) || new Map();
};

const updateLidCache = async (groupJid, groupMetadata) => {
    try {
        const mapping = createLidToJidMap(groupMetadata.participants);
        lidToJidCache.set(groupJid, mapping);
        return mapping;
    } catch (error) {
        console.error('Error updating LID cache:', error);
        return new Map();
    }
};
    ...
    if (jid.includes('@lid')) {
        if (jid.includes('-')) {
            const groupJid = jid.replace('@lid', '@g.us');
            const lidToJidMap = getCachedLidMapping(groupJid);
            targetJid = resolveJidFromLid(jid, lidToJidMap) || groupJid;
        } else {
            targetJid = jid.replace('@lid', '@s.whatsapp.net');
        }
    }
    
    const meId = authState.creds.me.id;
    let shouldIncludeDeviceIdentity = false;
    const { user, server } = (0, WABinary_1.jidDecode)(targetJid);
    const statusJid = 'status@broadcast';
    const isGroup = server === 'g.us';
    const isStatus = targetJid === statusJid;
    ...
    if (participant) {
        let participantJid = participant.jid;
        if (participantJid.includes('@lid')) {
            const groupJid = targetJid.includes('@g.us') ? targetJid : targetJid.replace('@lid', '@g.us');
            const lidToJidMap = getCachedLidMapping(groupJid);
            participantJid = resolveJidFromLid(participantJid, lidToJidMap) || participantJid.replace('@lid', '@s.whatsapp.net');
        }
    ...
    if ('productMessage' in content) {
        ...
        const msg = await Utils_1.generateWAMessageFromContent(targetJid, {
            viewOnceMessage: {
                message: {
                    interactiveMessage: {
                        body: { text: body },
        ...
        return await relayMessage(targetJid, msg.message, {
    ...
    const fullMsg = await Utils_1.generateWAMessage(targetJid, content, {
        logger,
        userJid,
        quoted,
        ...
        upload: async (readStream, opts) => {
            const up = await waUploadToServer(readStream, {
                ...opts,
                newsletter: WABinary_1.isJidNewsLetter(targetJid)
            });
            return up;
        },
    ...
    if (isDeleteMsg) {
        ...
        additionalAttributes.edit = (isGroup && !fromMe) || WABinary_1.isJidNewsLetter(targetJid) ? '8' : '7';
    } else if (isEditMsg) {
        additionalAttributes.edit = WABinary_1.isJidNewsLetter(targetJid) ? '3' : '1';
    } else if (isAiMsg) {
