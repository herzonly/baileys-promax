...
const { createLidToJidMap, resolveJidFromLid, processNodeLidToJid } = require('../Utils/generics');
...
const processNodeLidToJid = (node) => {
    if (!node || typeof node !== 'object') return node;
    if (node.attrs) {
        ['from', 'to', 'jid', 'lid', 'participant'].forEach(attr => {
            if (node.attrs[attr] && node.attrs[attr].includes('@lid')) {
                node.attrs[attr] = convertLidToJid(node.attrs[attr]);
            }
        });
    }
    ...
    frame = processNodeLidToJid(frame);
...
const query = async (node, timeoutMs) => {
    ...
    let lidToJidMap = new Map();
    
    if (groupJid && groupJid.includes('@g.us')) {
        lidToJidMap = getCachedLidMapping(groupJid);
    }
    
    node = processNodeLidToJid(node, lidToJidMap);
    ...
    noise.decodeFrame(data, frame => {
        ...
        if (!(frame instanceof Uint8Array)) {
            const groupJid = frame.attrs?.from;
            let lidToJidMap = new Map();
            ...
            frame = processNodeLidToJid(frame, lidToJidMap);
