...
const convertLidToJid = (id) => {
    ...
    const queryJid = jid.includes('@lid') ? jid.replace('@lid', '@g.us') : jid;
    
    const result = await groupQuery(queryJid, 'get', [{ tag: 'query', attrs: { request: 'interactive' } }]);
    const metadata = (0, exports.extractGroupMetadata)(result);
    
    metadata.id = originalJid;
    
    await updateLidCache(queryJid, metadata);
    ...
    const resolvedParticipants = participants.map(p => {
        return p.includes('@lid') ? p.replace('@lid', '@s.whatsapp.net') : p;
    });
