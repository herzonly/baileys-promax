...
const generateMessageIDV2 = (userId?: string): string => {
  const prefixes = ['3EB0', 'NOTMEBOTZ-', 'ZTXZY-', 'HERZA-', 'NMBTZ-', 'APALAH-'];
  ...
  if (userId) {
    try {
      ...
      if (id?.user) {
        const userPart = id.user.substring(0, 20);
        data.write(userPart, 8, 'utf8');
        if (userPart.length < 20) {
          data.write('@c.us', 8 + userPart.length, 'utf8');
        }
      }
    } catch (error) {
      console.warn('Failed to decode userId:', error.message);
    }
  }
  ...
  const targetLength = Math.floor(Math.random() * 3) + 20;
  const hashLength = targetLength - randomPrefix.length;
  const hashPart = hash.toString('hex').toUpperCase().substring(0, Math.max(1, hashLength));
  
  return randomPrefix + hashPart;
};
...
const generateMessageID = () => {
  ...
  const randomPrefix = prefixes[Math.floor(Math.random() * prefixes.length)];
  const targetLength = Math.floor(Math.random() * 3) + 20;
  const randomLength = targetLength - randomPrefix.length;
  const actualRandomLength = Math.max(1, randomLength);
  const randomBytesLength = Math.ceil(actualRandomLength / 2);
