...
const createLidToJidMap = (participants) => {
    const map = new Map();
    participants.forEach(participant => {
        if (participant.lid && participant.jid) {
            map.set(participant.lid, participant.jid);
        }
    });
    return map;
};

exports.createLidToJidMap = createLidToJidMap

const resolveJidFromLid = (lid, lidToJidMap) => {
    return lidToJidMap.get(lid) || lid;
};
exports.resolveJidFromLid = resolveJidFromLid

const processNodeLidToJid = (node, lidToJidMap = new Map()) => {
    if (!node || typeof node !== 'object') return node;
    
    if (node.attrs) {
        ['from', 'to', 'jid', 'participant'].forEach(attr => {
            if (node.attrs[attr] && node.attrs[attr].includes('@lid')) {
                const resolvedJid = resolveJidFromLid(node.attrs[attr], lidToJidMap);
                node.attrs[attr] = resolvedJid;
            }
        });
    }
    ...
    if (node.content && Array.isArray(node.content)) {
        node.content.forEach(child => {
            if (typeof child === 'object') {
                processNodeLidToJid(child, lidToJidMap);
            }
        });
    }
...
exports.processNodeLidToJid = processNodeLidToJid
